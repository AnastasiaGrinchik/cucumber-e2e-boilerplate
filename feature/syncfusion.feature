Feature: Syncfusion
  ...
  Scenario: Add new doctor to doctors tabs in syncfusion
    ...
    And select 'DoctorsForm > Department Select' from list '#Neurology in DropdownList'
    And type '$doctorsEducation' to 'DoctorsForm > Education Input'
    And type '$doctorsDesignation' to 'DoctorsForm > Designation Input'
    And select 'DoctorsForm > Experience Select' from list '#5+ years in DropdownList'
    And select 'DoctorsForm > Duty Timing Select' from list '#12:00 AM - 09:00 PM in DropdownList'
  ...
  Scenario: Add new patient to doctors tabs in syncfusion
    ...
    And select 'PatientsForm > BloodGroupSelect' from list '#O+ in DropdownList'
